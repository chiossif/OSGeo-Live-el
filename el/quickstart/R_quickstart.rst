...
πατήστε enter, και ο διερμηνευτής του R υπολογίζει/αποτιμά την εντολή και εκτυπώνει το αποτέλεσμα.
...
Και τα λοιπά. Μια μεγάλη συλλογή από αριθμητικές, τριγωνομετρικές και στατιστικές συναρτήσεις είναι διαθέσιμές σε πακέτα στη βιβλιοθήκη`CRAN <http://cran.r-project.org/>`_ archive.
...
αποθηκεύει πράγματα σε μονοδιάστατα διανύσματα καθώς και σε δισδιάστατους ή πολυδιάστατους πίνακες.. There are many functions that can 
generate these things. Εδώ είναι μια απλή σειρειοποίηση (sequence):
...
βάση δεδομένων, κάθε περίο πρέπει να είναι του ίδιου τύπου για κάθε εγγραφή. 
...
Μοιάζει λίγο συμπιεσμένο σε οποιονδήποτε ζει εκεί, καθώς είμαστε συνηθισμένοι με ένα σύστημα αναφοράς κεντραρισμένο στο γεωγραφικό μας πλάτος. Για την ώρα, το αντικείμενο δεν έχει ένα σύστημα αναφοράς προσαρτημένο σε αυτό, μπορούμε να το ελέγξουμε με μερικές ακόμα συναρτήσεις:
